        ...
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="rounded-md border p-3">
              ...
              <p className="text-lg font-semibold text-emerald-700">{data.successCount ?? 0}</p>
            </div>
            <div className="rounded-md border p-3">
              <p className="text-xs text-muted-foreground">Failed</p>
              <p className="text-lg font-semibold text-red-600">{data.failedCount ?? 0}</p>
            </div>
          </div>
              ...
              <div className="rounded-md border">
                {data.success.map((item, index) => (
                  ...
                  >
                    ...
                    <span className="text-xs text-muted-foreground">{item?.vendorType || item?.vendor_type || '-'}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-2">
                {data.failed.map((item, index) => (
                  <div key={`${item?.name || 'failed'}-${index}`} className="rounded-md border border-red-200 bg-red-50/40 p-3">
                    <p className="font-medium text-sm">{item?.name || 'Unknown Vendor'}</p>
