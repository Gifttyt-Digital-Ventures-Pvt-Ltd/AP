...
export const Login = () => {
  const isCorporateAuth = AUTH_PROVIDER === "corporate";
  ...
  const { login, register, getCorporatesByEmail, sendCorporateLoginOtp } =
    useAuth();
  const navigate = useNavigate();

  useEffect(() => {
    let timer = null;
    if (isCorporateAuth && isActive && seconds > 0) {
      timer = setInterval(() => setSeconds((prev) => prev - 1), 1000);
    } else if (seconds === 0) {
      setIsActive(false);
    }
    return () => clearInterval(timer);
  }, [isCorporateAuth, isActive, seconds]);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <Input
              ...
              type="email"
              value={email}
              disabled={isCorporateAuth && currentStep > 1}
