...
const DepartmentDialog = ({
  ...
  useEffect(() => {
    ...
    setMakerAssignedUserIds(department?.makerAssignedUserIds || []);
    setCheckerAssignedUserIds(department?.checkerAssignedUserIds || []);
  }, [department, open]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>
            {department
              ? "Update department details and assign invoice makers/checkers."
              : "Create a department and assign invoice makers/checkers for this department."}
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4 overflow-y-auto pr-1">
            ...
            <div className="flex items-center justify-between rounded-md border border-border px-4 py-3">
              <div className="space-y-1">
                <Label htmlFor="department-active">Active Department</Label>
                <p className="text-xs text-muted-foreground">
                  Inactive departments are hidden from invoice and workflow selection.
                </p>
              </div>
              <Switch
                ...
                data-testid="department-active-switch"
              />
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
              {renderAssignmentSection(
                ...
                "Select users who can create invoices in this department",
                makerEmployees,
                "No invoice makers available. Add an invoice maker before saving.",
                makerAssignedUserIds,
                handleMakerToggle,
                "department-maker-user",
              )}
              {renderAssignmentSection(
                "Assigned Invoice Checkers",
                "Select users who can check invoices in this department",
                checkerEmployees,
                "No invoice checkers available. Add an invoice checker before saving.",
                checkerAssignedUserIds,
                handleCheckerToggle,
                "department-checker-user",
              )}
            </div>
